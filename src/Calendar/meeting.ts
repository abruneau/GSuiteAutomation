...
export class Meeting {
  ...
  blockers(
    ...
    const start = new Date(this.event.end?.dateTime || '');
    const end = getRelativeDate(start, 0, 0, 10);
    return cal.getEvents(start, end, { search: 'block' });
  }

  note(): GoogleAppsScript.Drive.File | null {
    // First try to get note by stored file ID
    const noteId = this.event.extendedProperties?.private?.note;
    if (noteId) {
      try {
        return DriveApp.getFileById(noteId);
      } catch (e) {
        this.ctx.log.debug(
          `Note file with ID ${noteId} not found, falling back to filename search`
        );
      }
    }
    ...
    const fileName = this.fileName(
      this.event.originalStartTime?.dateTime || this.event.start?.dateTime || ''
    );
    const files = dir.getFilesByName(fileName);
    if (files.hasNext()) {
      const file = files.next();
      // Store the file ID for future reference
      this.storeNoteId(file.getId());
      return file;
    }
    return null;
  }
  ...
  private storeNoteId(fileId: string) {
    ...
    if (this.event.extendedProperties?.private) {
      this.event.extendedProperties.private['note'] = fileId;
    } else {
  ...
  attendees(): Email[] {
    if (!this.attendyList.length) {
      const emails =
        this.event.attendees?.map<string>(a => a.email || '') || [];
  ...
  isExternal(): boolean {
    const emails = this.attendees().filter(e => e.isExternal());
  ...
  colorize() {
    const evnt = CalendarApp.getDefaultCalendar().getEventById(
      this.event.id ?? ''
    );
