...
export function processMails(ctx: Context) {
    let threads = ctx.tbpLabel.getThreads()
...
function applyLabel(ctx: Context, thread: GoogleAppsScript.Gmail.GmailThread, labelName: string): GoogleAppsScript.Gmail.GmailThread {
    ctx.log.info(`Add Label ${labelName} to thread ${thread.getFirstMessageSubject()}`)
    if (ctx.DEBUG) {
        return thread
    } else {
        let accountLabel = GmailApp.getUserLabelByName(labelName) ? GmailApp.getUserLabelByName(labelName) : GmailApp.createLabel(labelName);
